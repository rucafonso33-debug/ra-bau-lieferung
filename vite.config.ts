  ...
  return {
    ...
    load(id) {
      ...
      return partFiles.map((file) => fs.readFileSync(file, 'utf8')).join('')
        ...
        .replace('placeholder="Produkt, Marke oder Referenz"', 'placeholder="Produkt, Stil, Marke oder Referenz"')
        ...
        .replace('Das Produkt steht im Mittelpunkt.', 'Materialien, die Räume aufwerten.')
        .replace('Bau- und Renovationsmaterial für projektbezogene Lieferungen in der Schweiz.', 'Kuratiert für anspruchsvolle Bau- und Renovationsprojekte in der Schweiz.')
        .replace('Sie senden Produkt, Referenz und Menge. Wir prüfen Herstellerdaten, Verfügbarkeit, Transport und Verzollung – vom Einzelprodukt bis zur Projektmenge.', 'Starten Sie beim Raum oder beim konkreten Produkt. Wir verbinden Materialwirkung, Herstellerreferenz, Menge und Schweizer Projektlogistik in einer klaren Anfrage.')
        .replace('<span>Reale Produkte</span><span>Werkdaten geprüft</span><span>Schweiz-Logistik</span><span>Persönlicher Kontakt</span>', '<span>Kuratiertes Sortiment</span><span>Herstellerreferenzen</span><span>Schweiz-Logistik</span><span>Persönlicher Kontakt</span>')
        .replace('Ein klarer Weg vom Produkt zur schriftlichen Offerte.', 'In wenigen Schritten zur geprüften Projektanfrage.')
        .replace('          <HeroSparten onNavigate={onNavigate} />\n        </div>\n      </section>\n\n      <section className="border-b border-[#d9e1e5] bg-[#10293e] text-white">', '          <HeroSparten onNavigate={onNavigate} />\n        </div>\n      </section>\n\n      <ProjectStart onNavigate={onNavigate} />\n\n      <section className="border-b border-[#d9e1e5] bg-[#10293e] text-white">')
        .replace('      </section>\n\n      <section className="mx-auto max-w-[1540px] px-5 py-12 lg:px-10 lg:py-16">', '      </section>\n\n      <CategoryAdvisor category={category} onNavigate={onNavigate} />\n\n      <section className="mx-auto max-w-[1540px] px-5 py-12 lg:px-10 lg:py-16">')
        .replace('      <Footer onNavigate={navigate} />', '      <MobileQuoteDock count={quote.length} onNavigate={navigate} />\n      <Footer onNavigate={navigate} />')
