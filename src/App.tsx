...
function Home({ onNavigate, featured, onSelect, onAdd }: { onNavigate: (p: PageRoute) => void; featured: InteriorProduct[]; onSelect: (p: InteriorProduct) => void; onAdd: (p: InteriorProduct) => void }) {
  return <main>
    <section className="relative overflow-hidden bg-[#132335] text-white">
      <div className="absolute inset-0 opacity-55"><img src={hires('/images/catalog/recer-pixstone.png')} className="h-full w-full object-cover" alt="Recer Pixstone Badezimmer" /><div className="absolute inset-0 bg-gradient-to-r from-[#132335] via-[#132335]/90 to-[#132335]/20" /></div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-6">
          {sparten.map((sparte, index) => <button key={sparte.number} onClick={() => onNavigate(sparte.route)} className={`group overflow-hidden rounded-[28px] border border-[#dedbd2] bg-[#f5f2eb] text-left shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl ${index < 2 ? 'xl:col-span-3' : 'xl:col-span-2'}`}>
            <div className="relative h-[300px] overflow-hidden bg-[#ece8df]">
              <img src={hires(sparte.image)} alt={sparte.title} className={`h-full w-full transition duration-700 group-hover:scale-105 ${sparte.contain ? 'object-contain p-8 mix-blend-multiply' : 'object-cover'}`} />
  ...
  return <main className="mx-auto max-w-[1480px] px-5 py-16 lg:px-10">
    ...
    <div className="mt-12 space-y-5">
      <button onClick={() => onNavigate('baustellenzubehoor')} className="group grid w-full overflow-hidden rounded-[30px] border border-[#dedbd2] bg-[#fffefb] text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl md:grid-cols-[340px_1fr_auto]">
        <div className="flex h-64 items-center justify-center bg-[#eeeae1] p-7 md:h-56"><img src={hires('/images/Komplettset.png')} alt="Nivellierclips, System-Keile und Systemzange" className="h-full w-full object-contain mix-blend-multiply" /></div>
      ...
      <img src={hires(cat.products[0].image)} alt="" className="h-64 w-full object-cover md:h-56" />
  ...
  return <main>
    <section className="bg-[#132335] text-white">
      <div className="mx-auto grid max-w-[1480px] gap-10 px-5 py-16 lg:grid-cols-[1fr_440px] lg:px-10 lg:py-20">
        ...
        <img src={hires(data.products.find(p => p.featured)?.image || data.products[0].image)} alt={meta.title} className="h-[320px] w-full rounded-[28px] object-cover" />
      </div>
    </section>
...
function ProductCard({ product, onSelect, onAdd, inQuote = false }: { key?: string; product: InteriorProduct; onSelect: (p: InteriorProduct) => void; onAdd: (p: InteriorProduct) => void; inQuote?: boolean }) {
  return <article className="group overflow-hidden rounded-[26px] border border-[#dedbd2] bg-[#fffefb] shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
    <button onClick={() => onSelect(product)} className="relative block h-[290px] w-full overflow-hidden bg-[#ebe8e0] text-left">
      <img src={hires(product.image)} alt={product.name} loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" />
...
function ProductModal({ product, inQuote, onClose, onAdd }: { product: InteriorProduct; inQuote: boolean; onClose: () => void; onAdd: () => void }) {
  return <div role="dialog" aria-modal="true" aria-label={`Produktdetails: ${product.name}`} className="fixed inset-0 z-[80] flex items-end justify-center bg-[#0a1420]/70 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <div className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-t-[30px] bg-[#fffefb] shadow-2xl sm:rounded-[30px]">
      <div className="grid lg:grid-cols-2"><div className="relative min-h-[340px] bg-[#ebe8e0]"><img src={hires(product.image)} alt={product.name} className="absolute inset-0 h-full w-full object-cover" /><button onClick={onClose} aria-label="Produktdetails schliessen" className="absolute left-4 top-4 rounded-full bg-white p-2 shadow"><X size={19} /></button></div>
